...
app.post('/buy', async (req, res) => {
  ...
  const stoc = [{ title: title, price: price, quantity: quantity }];
  ...
  try {
    ...
    for (let i = 0; i < user.stock.length; i++) {
      if (user.stock[i].title == title) {
        ...
        await User.findOneAndUpdate(
          ...
          {
            'stock.$.quantity': parseInt(quantity) + parseInt(newQuan),
            'stock.$.price': avg,
          }
        );
        flag = 1;
    ...
    console.log(newVal);
    await User.findOneAndUpdate({ _id: user._id }, { Wallet: newVal });
...
app.get('/temp', async (req, res) => {
  ...
  res.send(user);
  try {
    const stoc = [{ title: 'tesla', price: 55, quantity: 1 }];
    console.log(stoc.price * stoc.quantity);
    await User.findOneAndUpdate({ _id: user._id }, { $push: { stock: stoc } });
